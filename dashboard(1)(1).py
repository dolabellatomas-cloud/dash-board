...
def carregar_dados():
    np.random.seed(42)
    datas = pd.date_range("2020-01-01", "2026-12-31", freq="h")
    regioes = ["Sudeste", "Sul", "Nordeste", "Norte", "Centro-Oeste"]
    produtos = ["AirPods", "iPhone", "Apple Watch"]
...
# Agrupa por mês para o gráfico de linha não ficar poluído
df_filtrado["mes"] = df_filtrado["data"].dt.to_period("M").dt.to_timestamp()
vendas_por_mes = df_filtrado.groupby("mes")["vendas"].sum().reset_index()
fig3 = px.line(vendas_por_mes, x="mes", y="vendas", title="Vendas ao Longo do Tempo (por mês)")
...
st.dataframe(df_filtrado.drop(columns=["mes"]), use_container_width=True)
